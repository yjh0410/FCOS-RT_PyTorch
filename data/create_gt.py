...
def gt_creator(img_size, num_classes, strides, scale_range, label_lists=[]):
    ...
    # empty gt tensor
    for s in strides:
        gt_tensor.append(np.zeros([h//s, w//s, num_classes + 4 + 1]))
                    ...
                    if (j >= 0 and j < gt_tensor[si].shape[1]) and (i >= 0 and i < gt_tensor[si].shape[2]):
                        ...
                        if min(t, b, l, r) > 0:
                            if max(t, b, l, r) >= (sr[0]/s) and max(t, b, l, r) < (sr[1]/s):
                                gt_tensor[si][j, i, cls_id] = 1.0
                                gt_tensor[si][j, i, num_classes:num_classes + 4] = np.array([x1, y1, x2, y2])
                                gt_tensor[si][j, i, num_classes + 4] = np.sqrt(min(l, r) / max(l, r) * \
                                                                                    min(t, b) / max(t, b))
